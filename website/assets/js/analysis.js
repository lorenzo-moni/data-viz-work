...
function immortalsTop(n) {
  return GAMES_DATA
    .filter((g) => g.year <= 2018 && g.peak_players >= 1000 && g.engagement_total >= 30)
...
function fadingAAAGames() {
  return GAMES_DATA.filter(
    (g) => g.peak_players > 100000 && g.alive_ratio < 0.08
  ).sort((a, b) => d3.descending(a.peak_players, b.peak_players));
}

function slowBurnTop(n) {
  const candidates = GAMES_DATA.filter((g) =>
    g.peak_players < 100000 && g.alive_ratio > 0.10 &&
    g.engagement_total >= 20 && g.series.length >= 12
  );
  const shaped = candidates.filter((g) => {
    const relMs = new Date(g.year, g.release_month, 1).getTime();
    const peakIdx = g.series.reduce((mi, s, i) => s.players > g.series[mi].players ? i : mi, 0);
  ...
  return pool.sort((a, b) => d3.descending(a.current_players, b.current_players)).slice(0, n);
}
...
function renderAnalysisView() {
  ...
  d3.select("#act11-sort-chips").selectAll(".sort-chip").on("click", function () {
    d3.select("#act11-sort-chips").selectAll(".sort-chip").classed("active", false);
    d3.select(this).classed("active", true);
    initAct11(this.dataset.sort);
  });

  d3.select("#act15-sort-chips").selectAll(".sort-chip").on("click", function () {
    d3.select("#act15-sort-chips").selectAll(".sort-chip").classed("active", false);
    d3.select(this).classed("active", true);
...
function initAct1() {
  ...
  const aliveCount = games.filter((g) => g.alive_ratio > 0.1).length;

  d3.select("#act1-total").text(games.length.toLocaleString());
  d3.select("#act1-median").text((median * 100).toFixed(1) + "%");
  d3.select("#act1-alive").text(aliveCount.toLocaleString());
  ...
  const bins = d3.bin().domain([0, 1]).thresholds(20)(games.map((d) => d.alive_ratio));

  const x = d3.scaleLinear().domain([0, 1]).range([0, w]);
  const y = d3.scaleLog().domain([0.5, d3.max(bins, (b) => b.length) * 1.3]).range([h, 0]).clamp(true);

  g.append("g").attr("class", "axis axis-x").attr("transform", `translate(0,${h})`)
    .call(d3.axisBottom(x).ticks(5).tickFormat(d3.format(".0%")));
  g.append("g").attr("class", "axis axis-y")
    .call(d3.axisLeft(y).ticks(4).tickFormat((d) => d >= 1000 ? (d / 1000).toFixed(0) + "k" : d));
  ...
  g.append("line")
    ...
    .attr("y1", 0).attr("y2", h)
    .attr("stroke", "var(--ink-dim)").attr("stroke-dasharray", "4,3").attr("stroke-width", 1);
  g.append("text")
    .attr("x", x(median) + 4).attr("y", 12)
    .attr("fill", "var(--ink-dim)").attr("font-size", 9)
    .text("median " + (median * 100).toFixed(1) + "%");

  g.append("text").attr("class", "axis-label")
    .attr("x", w / 2).attr("y", h + 36).attr("text-anchor", "middle")
    .text("alive ratio  (currently playing ÷ playing+beaten+dropped)");
  g.append("text").attr("class", "axis-label")
    .attr("transform", "rotate(-90)").attr("x", -h / 2).attr("y", -50).attr("text-anchor", "middle")
    .text("# of games (log)");

  const brushSel = g.append("g").attr("class", "brush");
  const brushObj = d3.brushX()
    .extent([[0, 0], [w, h]])
    .on("brush end", (event) => {
      ...
      const sub = games.filter((d) => d.alive_ratio >= x0 && d.alive_ratio <= x1);
      ...
      d3.select("#act1-alive").text(sub.filter((d) => d.alive_ratio > 0.1).length.toLocaleString());
...
function initAct2() {
  ...
  const byCohort = d3.rollup(GAMES_DATA, (v) => ({
    count: v.length,
    median: d3.median(v, (d) => d.alive_ratio) || 0,
    top3: v.sort((a, b) => d3.descending(a.alive_ratio, b.alive_ratio)).slice(0, 3),
    all: v,
  }), (d) => d.year);
  ...
  const x = d3.scaleBand().domain(data.map((d) => d.year)).range([0, w]).padding(0.15);
  const y = d3.scaleLinear().domain([0, d3.max(data, (d) => d.count) * 1.1]).range([h, 0]);

  g.append("g").attr("class", "axis axis-x").attr("transform", `translate(0,${h})`)
    .call(d3.axisBottom(x).tickValues(data.filter((_, i) => i % 5 === 0).map((d) => d.year)).tickFormat(d3.format("d")));
  g.append("g").attr("class", "axis axis-y")
  ...
  g.append("g").attr("class", "grid grid-y")
    ...
    .selectAll("text").remove();
  g.append("text").attr("class", "axis-label")
    .attr("transform", "rotate(-90)").attr("x", -h / 2).attr("y", -42).attr("text-anchor", "middle")
    .text("# of releases");
  ...
  g.selectAll("rect.bar2")
    ...
    .on("click", function (_ev, d) {
      ...
      if (!panel.classList.contains("hidden") && title.textContent.startsWith(String(d.year))) {
      ...
      list.innerHTML = d.all
        .sort((a, b) => d3.descending(a.alive_ratio, b.alive_ratio))
        .map((g2) => `<div class="cohort-row" data-id="${g2.id}">
  ...
  g.append("text").attr("x", x(peak.year) + x.bandwidth() / 2)
    .attr("y", y(peak.count) - 6).attr("text-anchor", "middle")
    .attr("fill", "var(--ink-dim)").attr("font-size", 9)
...
function initAct3() {
  ...
  const yL = d3.scaleLinear().domain([0, d3.max(bins, (b) => b.length) * 1.1]).range([h, 0]);

  gL.append("g").attr("class", "axis axis-x").attr("transform", `translate(0,${h})`)
  ...
  gL.append("text").attr("class", "axis-label")
    .attr("transform", "rotate(-90)").attr("x", -h / 2).attr("y", -46).attr("text-anchor", "middle")
    .text("# of games");
  gL.append("text").attr("class", "axis-label").attr("x", half / 2).attr("y", h + 34).attr("text-anchor", "middle")
    .text("completion rate (story-driven games)");

  gL.selectAll("rect.bar3")
    .data(bins).join("rect").attr("class", "bar3")
    .attr("x", (b) => xL(b.x0) + 1)
    .attr("width", (b) => Math.max(0, xL(b.x1) - xL(b.x0) - 2))
    .attr("y", (b) => yL(b.length)).attr("height", (b) => h - yL(b.length))
    .attr("fill", "var(--accent)").attr("opacity", 0.7);

  gL.append("line")
    .attr("x1", xL(medC)).attr("x2", xL(medC)).attr("y1", 0).attr("y2", h)
    .attr("stroke", "var(--ink)").attr("stroke-dasharray", "4,3").attr("stroke-width", 1);
  gL.append("text").attr("x", xL(medC) + 4).attr("y", 14)
    .attr("fill", "var(--ink-dim)").attr("font-size", 9)
    .text("median " + (medC * 100).toFixed(1) + "%");

  const gR = svg.append("g").attr("transform", `translate(${margin.left + half + 40},${margin.top})`);

  const TOP_GENRES3 = [...new Set(GAMES_DATA.flatMap((d) => d.genres))]
    .map((genre) => ({ genre, count: GAMES_DATA.filter((d) => d.genres.includes(genre)).length }))
  ...
  const genreData = TOP_GENRES3.map((genre) => {
    return {
      genre,
      meds: Object.fromEntries(TYPES3.map((t) => {
        const sub = GAMES_DATA.filter((d) => d.genres.includes(genre) && d.game_type === t);
        return [t, d3.median(sub, (d) => d.completion_rate) || 0];
      })),
    };
  }).sort((a, b) => d3.descending(a.meds.story, b.meds.story));

  const maxMed = d3.max(genreData, (d) => Math.max(...TYPES3.map((t) => d.meds[t])));
  const xR = d3.scaleLinear().domain([0, maxMed * 1.1]).range([0, half]);
  const yR = d3.scaleBand().domain(genreData.map((d) => d.genre)).range([0, h]).padding(0.15);
  const subBand = d3.scaleBand().domain(TYPES3).range([0, yR.bandwidth()]).padding(0.06);

  gR.append("g").attr("class", "axis axis-x").attr("transform", `translate(0,${h})`)
    .call(d3.axisBottom(xR).ticks(4).tickFormat(d3.format(".0%")));
  gR.append("g").attr("class", "axis axis-y").call(d3.axisLeft(yR).tickSize(0))
    .select(".domain").remove();
  gR.append("text").attr("class", "axis-label").attr("x", half / 2).attr("y", h + 34).attr("text-anchor", "middle")
  ...
  genreData.forEach((d) => {
    ...
    TYPES3.forEach((t) => {
      ...
      gR.append("rect")
        .attr("x", 0).attr("y", yBase + subBand(t))
        .attr("width", xR(d.meds[t])).attr("height", subBand.bandwidth())
        .attr("fill", TYPE_COLOR[t]).attr("opacity", 0.75)
        .append("title").text(`${d.genre} ${TYPE_LABEL[t]}: ${(d.meds[t] * 100).toFixed(1)}%`);
    });
  });

  const leg3 = gR.append("g").attr("transform", `translate(0,${h + 46})`);
  TYPES3.forEach((t, i) => {
    leg3.append("rect").attr("x", i * 110).attr("width", 9).attr("height", 9).attr("fill", TYPE_COLOR[t]).attr("opacity", 0.75);
    leg3.append("text").attr("x", i * 110 + 12).attr("y", 8).attr("font-size", 8).attr("fill", "var(--ink-dim)").text(TYPE_LABEL[t]);
  });
}
...
function initAct4() {
  ...
  const filtered = GAMES_DATA.filter((g) => g.engagement_total >= 10 && g.completion_rate > 0 && g.alive_ratio > 0);
  ...
  const x = d3.scaleLinear().domain([0, d3.max(filtered, (d) => d.completion_rate) * 1.05]).range([0, w]);
  ...
  g.append("g").attr("class", "grid grid-y")
    .call(d3.axisLeft(y).ticks(5).tickSize(-w).tickFormat("")).selectAll("text").remove();
  ...
  g.selectAll(".quadrant-label").data(qlabels).join("text")
    .attr("class", "quadrant-label").attr("x", (d) => d.tx).attr("y", (d) => d.ty)
  ...
  g.append("text").attr("x", w - 4).attr("y", h - 6).attr("text-anchor", "end")
    .attr("fill", "var(--ink-faint)").attr("font-size", 10)
    .text(`r (story-driven) = ${rStory.toFixed(3)}`);

  g.append("text").attr("class", "axis-label")
    .attr("x", w / 2).attr("y", h + 40).attr("text-anchor", "middle").text("completion rate");
  g.append("text").attr("class", "axis-label")
    .attr("transform", "rotate(-90)").attr("x", -h / 2).attr("y", -42).attr("text-anchor", "middle")
    .text("alive ratio");
...
function initAct5() {
  ...
  if (!stats.length) {
    g.append("text").attr("x", w / 2).attr("y", h / 2).attr("text-anchor", "middle")
      .attr("fill", "var(--ink-dim)").text("Insufficient time-series data");
    return;
  }

  const x = d3.scaleLinear().domain([0, d3.max(stats, (d) => d.key)]).range([0, w]);
  const y = d3.scaleLinear().domain([0, Math.min(1, d3.max(stats, (d) => d.p90) * 1.15)]).range([h, 0]);

  g.append("g").attr("class", "axis axis-x").attr("transform", `translate(0,${h})`)
    .call(d3.axisBottom(x).ticks(8).tickFormat((d) => d % 12 === 0 ? `yr ${d / 12}` : ""));
  g.append("g").attr("class", "axis axis-y")
    .call(d3.axisLeft(y).ticks(5).tickFormat(d3.format(".0%")));
  g.append("g").attr("class", "grid grid-y")
    .call(d3.axisLeft(y).ticks(5).tickSize(-w).tickFormat("")).selectAll("text").remove();

  g.append("text").attr("class", "axis-label").attr("x", w / 2).attr("y", h + 38).attr("text-anchor", "middle").text("months since release");
  g.append("text").attr("class", "axis-label").attr("transform", "rotate(-90)").attr("x", -h / 2).attr("y", -48).attr("text-anchor", "middle").text("players / peak (ratio)");

  const area5 = d3.area().x((d) => x(d.key)).y0((d) => y(d.p25)).y1((d) => y(d.p90)).curve(d3.curveCatmullRom);
  const line5 = d3.line().x((d) => x(d.key)).y((d) => y(d.median)).curve(d3.curveCatmullRom);

  g.append("path").datum(stats).attr("fill", "var(--alive)").attr("opacity", 0.15).attr("d", area5);
  g.append("path").datum(stats).attr("fill", "none").attr("stroke", "var(--alive)").attr("stroke-width", 2).attr("d", line5);

  const lineP90 = d3.line().x((d) => x(d.key)).y((d) => y(d.p90)).curve(d3.curveCatmullRom);
  g.append("path").datum(stats).attr("fill", "none").attr("stroke", "var(--alive)").attr("stroke-width", 1).attr("stroke-dasharray", "4,3").attr("opacity", 0.6).attr("d", lineP90);

  g.append("text").attr("x", 4).attr("y", 12).attr("fill", "var(--ink-faint)").attr("font-size", 9)
  ...
  function drawOverlay(genre, color) {
    ...
    if (!st.length) return;
    const p = g.append("path").datum(st).attr("fill", "none").attr("stroke", color).attr("stroke-width", 2).attr("opacity", 0.85).attr("d", line5);
    overlayPaths.set(genre, p);
  }

  const chips = d3.select("#act5-genre-chips");
  chips.selectAll(".act-chip").data(TOP_GENRES).join("button")
    .attr("class", "act-chip").text((d) => d)
  ...
  d3.select("#act5-reset").on("click", () => {
    ...
    overlayPaths.clear();
    d3.select("#act5-genre-chips").selectAll(".act-chip").classed("active", false);
  });
}
...
function initAct6() {
  ...
  function drawMiniChart(container, genre, isExpanded) {
    ...
    const m = isExpanded ? { top: 20, right: 20, bottom: 36, left: 50 } : { top: 6, right: 4, bottom: 6, left: 4 };

    const innerW = cW - m.left - m.right;
    const innerH = cH - m.top - m.bottom;

    const svg6 = d3.select(container).append("svg")
      .attr("width", "100%").attr("height", cH).attr("viewBox", `0 0 ${cW} ${cH}`).attr("preserveAspectRatio", "xMidYMid meet");
    const gg = svg6.append("g").attr("transform", `translate(${m.left},${m.top})`);

    const x6 = d3.scaleLinear().domain([0, d3.max(stats, (d) => d.key)]).range([0, innerW]);
    const y6 = d3.scaleLinear().domain([0, Math.min(1, d3.max(stats, (d) => d.p90) * 1.2)]).range([innerH, 0]);
    const slope = slopeLabel(stats);

    if (isExpanded) {
      gg.append("g").attr("class", "axis axis-x").attr("transform", `translate(0,${innerH})`)
        .call(d3.axisBottom(x6).ticks(8).tickFormat((d) => d % 12 === 0 ? `yr ${d / 12}` : ""));
      gg.append("g").attr("class", "axis axis-y").call(d3.axisLeft(y6).ticks(4).tickFormat(d3.format(".0%")));
      gg.append("text").attr("class", "axis-label").attr("x", innerW / 2).attr("y", innerH + 30).attr("text-anchor", "middle").text("months since release");
      gg.append("text").attr("class", "axis-label").attr("transform", "rotate(-90)").attr("x", -innerH / 2).attr("y", -40).attr("text-anchor", "middle").text("players / peak");
    } else if (slope) {
      gg.append("text").attr("x", innerW / 2).attr("y", innerH - 2).attr("text-anchor", "middle")
        .attr("fill", "var(--ink-faint)").attr("font-size", 8).text(slope);
    }

    const area6 = d3.area().x((d) => x6(d.key)).y0((d) => y6(d.p25)).y1((d) => y6(d.p90)).curve(d3.curveCatmullRom);
    const line6 = d3.line().x((d) => x6(d.key)).y((d) => y6(d.median)).curve(d3.curveCatmullRom);
    gg.append("path").datum(stats).attr("fill", "var(--alive)").attr("opacity", 0.15).attr("d", area6);
    gg.append("path").datum(stats).attr("fill", "none").attr("stroke", "var(--alive)").attr("stroke-width", isExpanded ? 2 : 1.5).attr("d", line6);
  }
  ...
  function renderMiniCards() {
    ...
    TOP_GENRES.forEach((genre) => {
      ...
      cell.className = "genre-mini" + (genre === expandedGenre ? " expanded" : "");
...
function initAct7() {
  ...
  function computeLift(dim) {
    const immortals = GAMES_DATA.filter((d) => d.year <= 2018 && d.alive_ratio > 0.10 && d.engagement_total >= 30);
    const mortals = GAMES_DATA.filter((d) => d.year <= 2018 && d.alive_ratio <= 0.03 && d.engagement_total >= 10);
    ...
    const keys = dim === "genre"
      ? [...new Set(GAMES_DATA.flatMap((d) => d.genres))].filter((k) => k !== "Unknown")
    ...
    return keys
      .map((k) => {
        ...
        return { key: k, lift: share(immortals, k) - share(mortals, k), nImm, nMort };
      })
  ...
  function render7(dim) {
    ...
    const y7 = d3.scaleBand().domain(data.map((d) => d.key)).range([0, h]).padding(0.25);
    const maxL = d3.max(data, (d) => Math.abs(d.lift));
    const x7 = d3.scaleLinear().domain([-maxL, maxL]).range([0, w]);

    g.append("g").attr("class", "axis axis-x").attr("transform", `translate(0,${h})`)
      .call(d3.axisBottom(x7).ticks(5).tickFormat((d) => d3.format("+.0%")(d)));
    g.append("g").attr("class", "axis axis-y").call(d3.axisLeft(y7).tickSize(0))
      .select(".domain").remove();

    g.append("line").attr("x1", x7(0)).attr("x2", x7(0)).attr("y1", 0).attr("y2", h)
      .attr("stroke", "var(--rule-strong)").attr("stroke-width", 1);

    g.selectAll("rect.bar7").data(data).join("rect").attr("class", "bar7")
      ...
      .on("click", (_ev, d) => {
        ...
        const immortals = GAMES_DATA.filter((gd) => gd.year <= 2018 && gd.alive_ratio > 0.10
          && gd.engagement_total >= 30
          && (dim === "genre" ? gd.genres.includes(d.key) : gd.platforms.includes(d.key))
        ).sort((a, b) => d3.descending(a.alive_ratio, b.alive_ratio)).slice(0, 10);
        drill.innerHTML = immortals.length
          ? `<div style="font:9px var(--mono);text-transform:uppercase;letter-spacing:.1em;color:var(--accent);margin-bottom:8px">Immortals in ${d.key} (N=${d.nImm})</div>` +
            immortals.map((gd, i) => `<div class="drill-row"><span class="drill-rank">${i + 1}</span><span class="drill-name">${gd.name}</span><span class="drill-alive">${(gd.alive_ratio * 100).toFixed(0)}%</span></div>`).join("")
          : `<p class="story-caption">No immortals found for ${d.key}.</p>`;
        document.getElementById("act7-guide")?.classList.add("done");
      });

    g.append("text").attr("class", "axis-label").attr("x", w / 2).attr("y", h + 34).attr("text-anchor", "middle")
...
function initAct8() {
  ...
  const metaGames = GAMES_DATA.filter((g) => g.metacritic > 0 && g.alive_ratio >= 0);
  const ratingGames = GAMES_DATA.filter((g) => g.rating > 0 && g.alive_ratio >= 0);
  const rMeta = pearsonR(metaGames, (d) => d.metacritic, (d) => d.alive_ratio);
  const rRating = pearsonR(ratingGames, (d) => d.rating, (d) => d.alive_ratio);
  ...
  function drawPanel(parent, data, xFn, xDomain, xLabel, rVal) {
    ...
    parent.append("defs").append("clipPath").attr("id", clipId)
      .append("rect").attr("x", 0).attr("y", 0).attr("width", half).attr("height", h);
    ...
    gg.append("g").attr("class", "axis axis-x").attr("transform", `translate(0,${h})`)
      .call(d3.axisBottom(x8).ticks(5));
    gg.append("g").attr("class", "axis axis-y").call(d3.axisLeft(y8).ticks(5).tickFormat(d3.format(".0%")));
    gg.append("text").attr("class", "axis-label").attr("transform", "rotate(-90)")
      .attr("x", -h / 2).attr("y", -46).attr("text-anchor", "middle").text("alive ratio");
    gg.append("line").attr("x1", x8(mx8)).attr("x2", x8(mx8)).attr("y1", 0).attr("y2", h)
      .attr("stroke", "var(--rule-strong)").attr("stroke-dasharray", "3,3");
    gg.append("line").attr("x1", 0).attr("x2", half).attr("y1", y8(my8)).attr("y2", y8(my8))
      .attr("stroke", "var(--rule-strong)").attr("stroke-dasharray", "3,3");

    const quadClip = gg.append("g").attr("clip-path", `url(#${clipId})`);
    const quadrants = [
      { id: "TL", x0: xDomain[0], x1: mx8, y0: my8, y1: 1, label: "Low score, high survival" },
      { id: "TR", x0: mx8, x1: xDomain[1], y0: my8, y1: 1, label: "High score, high survival" },
      { id: "BL", x0: xDomain[0], x1: mx8, y0: 0, y1: my8, label: "Low score, low survival" },
      { id: "BR", x0: mx8, x1: xDomain[1], y0: 0, y1: my8, label: "High score, low survival" },
    ];
    quadClip.selectAll(".quadrant-overlay").data(quadrants).join("rect")
      .attr("class", "quadrant-overlay")
      .attr("x", (d) => x8(d.x0)).attr("y", (d) => y8(d.y1))
      .attr("width", (d) => Math.max(0, x8(d.x1) - x8(d.x0))).attr("height", (d) => Math.max(0, y8(d.y0) - y8(d.y1)))
      .on("click", function (_ev, d) {
        const sel = data.filter((gd) => xFn(gd) >= d.x0 && xFn(gd) <= d.x1
          && gd.alive_ratio >= d.y0 && gd.alive_ratio <= d.y1)
          .sort((a, b) => d3.descending(a.alive_ratio, b.alive_ratio)).slice(0, 15);
        gg.selectAll(".quadrant-overlay").classed("active", false);
        d3.select(this).classed("active", true);
        drill8.innerHTML = `<div style="font:9px var(--mono);text-transform:uppercase;letter-spacing:.1em;color:var(--accent);margin-bottom:8px">${d.label} (${sel.length} games)</div>` +
          sel.map((gd, i) => `<div class="drill-row"><span class="drill-rank">${i + 1}</span><span class="drill-name">${gd.name}</span><span class="drill-alive">${(gd.alive_ratio * 100).toFixed(0)}%</span></div>`).join("");
      });

    gg.selectAll("circle.dot8")
      .data(data.filter((_, i) => i % 2 === 0))
      .join("circle").attr("class", "dot8")
      .attr("cx", (d) => x8(xFn(d))).attr("cy", (d) => y8(d.alive_ratio))
      .attr("r", 2.5).attr("fill", (d) => ARCHETYPE_COLOR[d.archetype]).attr("opacity", 0.4)
      .on("mouseover", (event, d) => {
        d3.select("#tooltip").classed("visible", true)
    ...
    gg.append("text").attr("x", half - 4).attr("y", h - 8).attr("text-anchor", "end")
      .attr("fill", "var(--ink-faint)").attr("font-size", 10).text(`r = ${rVal.toFixed(3)}`);
    gg.append("text").attr("class", "axis-label").attr("x", half / 2).attr("y", h + 36)
  ...
  const gL = svg.append("g").attr("transform", `translate(${margin.left},${margin.top})`);
  drawPanel(gL, metaGames, (d) => d.metacritic, [0, 100], "Metacritic score", rMeta);
  const gR = svg.append("g").attr("transform", `translate(${margin.left + half + 40},${margin.top})`);
  drawPanel(gR, ratingGames, (d) => d.rating, [1, 5], "RAWG rating (0–5)", rRating);
}
...
function initAct9() {
  ...
  const TOP_GENRES9 = ["All", ...[...new Set(GAMES_DATA.flatMap((d) => d.genres))]
    .map((genre) => ({ genre, count: GAMES_DATA.filter((d) => d.genres.includes(genre)).length }))
    .sort((a, b) => d3.descending(a.count, b.count)).slice(0, 8).map((d) => d.genre)];
  ...
  function render9(genre) {
    g.selectAll("*").remove();
    const sub = genre === "All" ? GAMES_DATA : GAMES_DATA.filter((d) => d.genres.includes(genre));
    const data9 = PRICE_BUCKETS.map((b) => {
      const bucket = sub.filter((d) => b.test(d.price));
      return { label: b.label, med: d3.median(bucket, (d) => d.alive_ratio) || 0, count: bucket.length };
    });

    const x9 = d3.scaleBand().domain(data9.map((d) => d.label)).range([0, w]).padding(0.3);
    const y9 = d3.scaleLinear().domain([0, d3.max(data9, (d) => d.med) * 1.3]).range([h, 0]);
    ...
    g.append("g").attr("class", "grid grid-y")
      .call(d3.axisLeft(y9).ticks(5).tickSize(-w).tickFormat("")).selectAll("text").remove();
    g.append("text").attr("class", "axis-label")
      .attr("transform", "rotate(-90)").attr("x", -h / 2).attr("y", -46).attr("text-anchor", "middle")
      .text("median alive ratio");

    g.selectAll("rect.bar9").data(data9).join("rect").attr("class", "bar9")
      .attr("x", (d) => x9(d.label)).attr("width", x9.bandwidth())
      .attr("y", (d) => y9(d.med)).attr("height", (d) => h - y9(d.med))
      .attr("fill", "var(--accent)").attr("opacity", 0.8);

    g.selectAll("text.bar9-count").data(data9).join("text").attr("class", "bar9-count")
      .attr("x", (d) => x9(d.label) + x9.bandwidth() / 2).attr("y", (d) => y9(d.med) - 5)
      .attr("text-anchor", "middle").attr("fill", "var(--ink-faint)").attr("font-size", 9)
      .text((d) => d.count > 0 ? `n=${d.count}` : "");
  }

  render9("All");
  sel9.addEventListener("change", () => render9(sel9.value));
}
  ...
  function render10() {
    ...
    const data10 = TOP_PLATFORMS.map((plat) => {
      ...
      const byArch = ARCHETYPES.map((arch) => {
        ...
        return { arch, med: d3.median(sub, (d) => d.alive_ratio) || 0, count: sub.length };
    ...
    g.append("g").attr("class", "axis axis-y").call(d3.axisLeft(y10).tickSize(0)).select(".domain").remove();
    ...
    data10.forEach((row) => {
      ...
      row.byArch.forEach((b) => {
        g.append("rect")
          .attr("x", 0).attr("y", yOffset + subGroupScale(b.arch))
          .attr("width", x10(b.med)).attr("height", subGroupScale.bandwidth())
          .attr("fill", ARCHETYPE_COLOR[b.arch]).attr("opacity", 0.8);
      });
    });
  }
...
function initAct11(sortKey = "alive") {
  ...
  if (sortKey === "peak") data11 = [...data11].sort((a, b) => d3.descending(a.peak_players, b.peak_players));
  else if (sortKey === "age") data11 = [...data11].sort((a, b) => d3.ascending(a.year, b.year));

  const container = document.getElementById("act11-list");
  container.innerHTML = data11.map((g) => `
...
function initAct12() {
  ...
  const games12 = GAMES_DATA.filter((d) => d.peak_players > 1000 && d.alive_ratio >= 0);
  const x12 = d3.scaleLog().domain([1000, d3.max(games12, (d) => d.peak_players) * 1.3]).range([0, w]);
  const y12 = d3.scaleLinear().domain([0, 1]).range([h, 0]);
  const PEAK_THRESHOLD = 100000, ALIVE_THRESHOLD = 0.08;

  g.append("g").attr("class", "axis axis-x").attr("transform", `translate(0,${h})`)
    .call(d3.axisBottom(x12).ticks(5, "~s"));
  g.append("g").attr("class", "axis axis-y")
    .call(d3.axisLeft(y12).ticks(5).tickFormat(d3.format(".0%")));
  g.append("text").attr("class", "axis-label").attr("x", w / 2).attr("y", h + 38).attr("text-anchor", "middle").text("peak players (log scale)");
  g.append("text").attr("class", "axis-label").attr("transform", "rotate(-90)").attr("x", -h / 2).attr("y", -42).attr("text-anchor", "middle").text("alive ratio");

  g.append("rect")
    .attr("x", x12(PEAK_THRESHOLD)).attr("y", y12(ALIVE_THRESHOLD))
    .attr("width", w - x12(PEAK_THRESHOLD)).attr("height", h - y12(ALIVE_THRESHOLD))
    .attr("fill", "rgba(217,108,108,0.08)").attr("stroke", "var(--dying)").attr("stroke-width", 1).attr("stroke-dasharray", "4,3");
  g.append("text").attr("class", "quadrant-label")
    .attr("x", x12(PEAK_THRESHOLD) + 8).attr("y", h - 8).text("FADING AAA");

  g.selectAll("circle.dot12")
    .data(games12, (d) => d.id)
    .join("circle").attr("class", "dot12")
  ...
  function renderCards12(pool) {
    const top4 = pool.sort((a, b) => d3.descending(a.peak_players, b.peak_players)).slice(0, 4);
    d3.select("#act12-cards").html(top4.map((gd) => `
      <div class="case-card">
  ...
  const brush12 = d3.brush().extent([[0, 0], [w, h]])
    .on("end", (event) => {
      const guide = document.getElementById("act12-guide");
      if (!event.selection) { renderCards12(fadingAAAGames()); return; }
      const [[bx0, by0], [bx1, by1]] = event.selection;
      const sel = games12.filter((d) =>
        x12(d.peak_players) >= bx0 && x12(d.peak_players) <= bx1 &&
        y12(d.alive_ratio) >= by0 && y12(d.alive_ratio) <= by1
      );
...
function initAct13() {
  ...
  const maxP = d3.max(anchors.flatMap((g) => g.series), (d) => d.players) * 1.2;
  ...
  anchors.forEach((game, idx) => {
    ...
    requestAnimationFrame(() => {
      ...
      const g13 = svg13.append("g").attr("transform", `translate(${margin13.left},${margin13.top})`);

      const x13 = d3.scaleTime().domain(tDomain).range([0, w13]);
      const y13 = d3.scaleLog().domain([Math.max(1, d3.min(game.series, (d) => d.players) || 1), maxP]).range([h13, 0]).clamp(true);

      g13.append("g").attr("class", "axis axis-x").attr("transform", `translate(0,${h13})`)
        .call(d3.axisBottom(x13).ticks(3).tickFormat(d3.timeFormat("%Y")));
      g13.append("g").attr("class", "axis axis-y")
        .call(d3.axisLeft(y13).ticks(2).tickFormat((d) => d >= 1000 ? (d / 1000).toFixed(0) + "k" : d));
      if (idx % 3 === 0) {
        g13.append("text").attr("class", "axis-label")
          .attr("transform", "rotate(-90)").attr("x", -h13 / 2).attr("y", -28).attr("text-anchor", "middle")
          .attr("font-size", 7).text("avg players/mo");
      }

      const line13 = d3.line().x((d) => x13(d.month)).y((d) => y13(Math.max(1, d.players))).curve(d3.curveCatmullRom);
      const color13 = TS_PALETTE[idx % TS_PALETTE.length];
      g13.append("path").datum(game.series).attr("fill", "none").attr("stroke", color13).attr("stroke-width", 1.5).attr("d", line13);

      const xLine13 = g13.append("line").attr("y1", 0).attr("y2", h13)
        .attr("stroke", "var(--ink-dim)").attr("stroke-width", 1).attr("stroke-dasharray", "3,3").attr("opacity", 0).attr("pointer-events", "none");

      panels.push({ g: g13, x: x13, y: y13, xLine: xLine13, series: game.series, w: w13, h: h13 });

      g13.append("rect")
        .attr("width", w13).attr("height", h13).attr("fill", "transparent")
        .on("mousemove", function (event) {
...
function initAct14() {
  ...
  const games14 = GAMES_DATA.filter((g) => g.youtube_count > 0 && g.youtube_count < 1000000 && g.year < 2026);
  const medAlive = d3.median(GAMES_DATA, (g) => g.alive_ratio) || 0;
  const high = games14.filter((g) => g.alive_ratio >= medAlive).map((g) => g.youtube_count / Math.max(1, 2026 - g.year));
  const low = games14.filter((g) => g.alive_ratio < medAlive).map((g) => g.youtube_count / Math.max(1, 2026 - g.year));

  const svg14 = d3.select(svgEl);
  svg14.selectAll("*").remove();
  const g = svg14.append("g").attr("transform", `translate(${margin.left},${margin.top})`);

  const allVals = [...high, ...low].filter((v) => v > 0);
  const xExtent = d3.extent(allVals);
  const x14 = d3.scaleLog().domain([Math.max(1, xExtent[0]), xExtent[1]]).range([0, w]).clamp(true);
  ...
  const y14 = d3.scaleLinear().domain([0, d3.max([...densityHigh, ...densityLow], (d) => d[1]) * 1.2]).range([h, 0]);

  g.append("g").attr("class", "axis axis-x").attr("transform", `translate(0,${h})`)
    .call(d3.axisBottom(x14).ticks(5, "~s"));
  g.append("g").attr("class", "axis axis-y").call(d3.axisLeft(y14).ticks(4));
  g.append("g").attr("class", "grid grid-y")
    .call(d3.axisLeft(y14).ticks(4).tickSize(-w).tickFormat("")).selectAll("text").remove();
  g.append("text").attr("class", "axis-label").attr("x", w / 2).attr("y", h + 38).attr("text-anchor", "middle").text("YouTube videos / year (log scale, capped rows excluded)");
  g.append("text").attr("class", "axis-label").attr("transform", "rotate(-90)").attr("x", -h / 2).attr("y", -46).attr("text-anchor", "middle").text("density");

  const lineFn = d3.line().x((d) => x14(Math.pow(10, d[0]))).y((d) => y14(d[1])).curve(d3.curveBasis);
  g.append("path").datum(densityLow).attr("fill", "none").attr("stroke", "var(--dying)").attr("stroke-width", 2.5).attr("opacity", 0.85).attr("d", lineFn);
  g.append("path").datum(densityHigh).attr("fill", "none").attr("stroke", "var(--alive)").attr("stroke-width", 2.5).attr("opacity", 0.85).attr("d", lineFn);
  ...
  g.append("line").attr("x1", x14(medL)).attr("x2", x14(medL)).attr("y1", 0).attr("y2", h)
    .attr("stroke", "var(--dying)").attr("stroke-dasharray", "4,3").attr("stroke-width", 1.5);
  g.append("text").attr("x", x14(medH) + 4).attr("y", 12).attr("fill", "var(--alive)").attr("font-size", 9)
    .text(Math.round(medH).toLocaleString() + "/yr");
  g.append("text").attr("x", x14(medL) + 4).attr("y", 24).attr("fill", "var(--dying)").attr("font-size", 9)
    .text(Math.round(medL).toLocaleString() + "/yr");

  const leg = g.append("g").attr("transform", `translate(${w - 140},8)`);
  [{ color: "var(--alive)", label: "High vitality" }, { color: "var(--dying)", label: "Low vitality" }].forEach((item, i) => {
    leg.append("rect").attr("x", 0).attr("y", i * 16).attr("width", 14).attr("height", 3).attr("fill", item.color);
    leg.append("text").attr("x", 18).attr("y", i * 16 + 3).attr("fill", "var(--ink-dim)").attr("font-size", 10).text(item.label);
  });
}
...
function initAct15() {
  const games15 = mortalityIndex(
    GAMES_DATA.filter((g) => g.peak_players >= 1000 && g.engagement_total >= 100)
  );
...
function renderAct15List(sortKey, games15Arg) {
  ...
  list.querySelectorAll(".mortality-row").forEach((row) => {
    row.addEventListener("click", () => {
      const gm = GAMES_DATA.find((x) => x.id === +row.dataset.id);
      if (gm) { addToSelection(gm); switchView("sandbox"); }
    });
  });
...
function makeSpark(g) {
  ...
  const xS = d3.scaleLinear().domain(d3.extent(g.series, (d) => d.month)).range([0, w]);
  const yS = d3.scaleLinear().domain([0, maxP]).range([sh - 2, 2]);
  const pts = g.series.map((d) => `${xS(d.month).toFixed(1)},${yS(d.players).toFixed(1)}`).join(" ");

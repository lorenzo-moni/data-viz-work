...
let _controlsOriginalParent = null;
let _controlsOriginalNextSibling = null;
...
function _onFullscreenChange() {
  ...
  for (const [wrapEl, opts] of _toolbarRegistry) {
    ...
    // Re-parent the landscape filter panel
    if (opts.hasOverlayPanel) {
      const controls = document.querySelector(".controls");
      if (isFullscreen && controls) {
        _controlsOriginalParent = controls.parentNode;
        _controlsOriginalNextSibling = controls.nextSibling;
        wrapEl.appendChild(controls);
        controls.classList.add("is-fullscreen-controls");
      } else if (!isFullscreen && controls && controls.classList.contains("is-fullscreen-controls")) {
        _controlsOriginalParent.insertBefore(controls, _controlsOriginalNextSibling);
        controls.classList.remove("is-fullscreen-controls");
      }
    }

...
Cleans rawg-games-dataset (890K rows, 54 cols) down to ~10-20K usable games.
"""
...
# Normalize nulls ("nan" strings, empty strings → NaN)
...
print(f"Filtered: {n_before:,} → {len(df):,} games ({len(df)/n_before*100:.1f}% kept)")

# Parse ESRB rating (JSON dict → name string)
def parse_esrb(val):
    if pd.isna(val):
        return np.nan
    try:
...
# Parse added_by_status → separate columns
...
# Parse ratings distribution (pipe-delimited pseudo-JSON → pct columns)
...
print(f"Exported {len(df)} rows × {df.shape[1]} cols → rawg_clean.csv")

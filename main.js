...
function _pickAnalysisPairs() {
  ...
  const immortals = GAMES_DATA.filter(
    (g) => g.archetype === "immortal" && hasSeries(g),
  );
  const fadingAAA = GAMES_DATA.filter(
    (g) => g.archetype === "fading_aaa" && hasSeries(g),
  );
  const slowBurn = GAMES_DATA.filter(
    (g) => g.archetype === "slow_burn" && hasSeries(g),
  );
...
function renderAnalysisCharts() {
  ...
  pairs.forEach(([aId, bId], i) => {
    ...
    if (a && b && legends[i]) {
      ...
      if (spans[0])
        spans[0].innerHTML = `<span class="dot dot-alive"></span> <em>${a.name}</em> (${a.year})`;
      if (spans[1])
        spans[1].innerHTML = `<span class="dot dot-dying"></span> <em>${b.name}</em> (${b.year})`;
    }
  });
}

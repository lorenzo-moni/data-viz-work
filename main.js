...
function switchView(targetView) {
  ...
  d3.select('#selection-tray')
    .classed('hidden', targetView !== 'main' || state.selectedIds.size === 0);
  ...
  if (targetView === 'analysis') {
    requestAnimationFrame(() => renderAnalysisCharts());
  }
...
function getFilteredGames() {
  return GAMES_DATA.filter(g => {
    if (g.year < state.yearRange[0] || g.year > state.yearRange[1]) return false;
    if (g.rating < state.minRating) return false;
    if (state.genres.size > 0 && !g.genres.some(gen => state.genres.has(gen))) return false;
    if (state.platforms.size > 0 && !g.platforms.some(p => state.platforms.has(p))) return false;
    return true;
  });
}
...
const chart = {
  svg: null, g: null, xScale: null, yScale: null, rScale: null, colorScale: null,
  width: 0, height: 0, margin: { top: 30, right: 40, bottom: 60, left: 80 },
};
...
function initChart() {
  ...
  chart.g.append('g').attr('class', 'axis axis-x').attr('transform', `translate(0,${chart.height})`);
...
function renderChart(games) {
  ...
  chart.g.select('.axis-x')
    .transition().duration(500)
    .call(d3.axisBottom(chart.xScale).tickFormat(d3.format('d')).ticks(Math.min(10, state.yearRange[1] - state.yearRange[0])));

  chart.g.select('.axis-y')
    .transition().duration(500)
    .call(d3.axisLeft(chart.yScale).ticks(6).tickFormat(d => {
  ...
  chart.g.select('.grid-y')
    .transition().duration(500)
    .call(d3.axisLeft(chart.yScale).ticks(6).tickSize(-chart.width).tickFormat(''))
    .selectAll('text').remove();

  const bubbles = chart.g.select('.bubbles')
...
function onHoverOut() { tooltip.classed('visible', false); }
...
function showCard(d) {
  ...
  d3.select('#card-players').text(`${fmtPlayers(d.avg_players)} / ${fmtPlayers(d.peak_players)}`);
...
function updateSelectionUI() {
  ...
  d3.select('#nav-sandbox')
    .classed('disabled', count === 0);
  ...
  // Tray chips
  const selectedGames = [...state.selectedIds].map(id => GAMES_DATA.find(g => g.id === id));
  d3.select('#tray-chips')
    .selectAll('.tray-chip')
    .data(selectedGames, d => d.id)
    .join(
      enter => enter.append('div').attr('class', 'tray-chip').text(d => d.name),
  ...
  d3.select('#selection-chips')
    ...
    .join(
      enter => {
        ...
        e.append('span').attr('class', 'sel-name').text(d => d.name);
        e.append('button').attr('class', 'sel-remove').text('×')
          .on('click', (event, d) => {
...
const sandbox = {
  svg: null, g: null, xScale: null, yScale: null,
  width: 0, height: 0, margin: { top: 30, right: 40, bottom: 60, left: 80 },
...
function initSandbox() {
  ...
  // Nuke any stale content from previous init
  svg.selectAll('*').remove();

  sandbox.g = svg.append('g')
    .attr('transform', `translate(${sandbox.margin.left},${sandbox.margin.top})`);
  sandbox.g.append('g').attr('class', 'axis axis-x').attr('transform', `translate(0,${sandbox.height})`);
  sandbox.g.append('g').attr('class', 'axis axis-y');
  sandbox.g.append('g').attr('class', 'grid grid-y');
  sandbox.g.append('text').attr('class', 'axis-label sandbox-x-label')
    .attr('x', sandbox.width / 2).attr('y', sandbox.height + 48).attr('text-anchor', 'middle');
  sandbox.g.append('text').attr('class', 'axis-label sandbox-y-label')
    .attr('transform', 'rotate(-90)').attr('x', -sandbox.height / 2).attr('y', -60).attr('text-anchor', 'middle');
...
function renderSandbox() {
  ...
  const selected = [...state.selectedIds].map(id => GAMES_DATA.find(g => g.id === id));
  ...
  function paddedDomain(vals, isLog = false) {
    ...
    let hi = d3.max(vals);
    if (lo === hi) {
      // Single value or all equal — make up a sensible window around it
  ...
  sandbox.g.select('.axis-x')
    .transition().duration(400)
    .call(d3.axisBottom(sandbox.xScale).ticks(6));
  sandbox.g.select('.axis-y')
    .transition().duration(400)
    .call(d3.axisLeft(sandbox.yScale).ticks(6).tickFormat(d => {
      if (d >= 1e6) return (d / 1e6).toFixed(1) + 'M';
      if (d >= 1e3) return (d / 1e3).toFixed(0) + 'k';
  ...
  sandbox.g.select('.sandbox-x-label').text(FIELD_LABELS[sandbox.xField].toLowerCase());
  sandbox.g.select('.sandbox-y-label').text(FIELD_LABELS[sandbox.yField].toLowerCase());

  // Points + labels
  const points = sandbox.g.select('.sandbox-points')
...
d3.selectAll('#sandbox-x, #sandbox-y, #sandbox-color').on('change', function() {
...
// ==========================================================
// ANALYSIS: side-by-side time-series comparisons
// ==========================================================

function drawComparisonChart(selector, gameAId, gameBId) {
  ...
  const a = GAMES_DATA.find(g => g.id === gameAId);
  const b = GAMES_DATA.find(g => g.id === gameBId);
  if (!a || !b) return;

  const allSeries = [...a.series, ...b.series];
  const x = d3.scaleTime()
    .domain(d3.extent(allSeries, d => d.month))
    .range([0, iw]);
  const y = d3.scaleLinear()
    .domain([0, d3.max(allSeries, d => d.players) * 1.1])
    .range([ih, 0]);

  g.append('g').attr('transform', `translate(0,${ih})`)
    .call(d3.axisBottom(x).ticks(4).tickFormat(d3.timeFormat('%Y')))
    .call(sel => sel.selectAll('text').attr('class', 'axis-tick-label'));
  g.append('g')
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  setupSliders();
  initChart();
  // Sandbox is lazy-initialized on first view switch (must be visible for bbox)
  update();
  updateSelectionUI();
});
...
window.addEventListener('resize', () => {
  ...
  resizeTimer = setTimeout(() => {
    ...
    if (state.view === 'analysis') renderAnalysisCharts();
  }, 200);
});